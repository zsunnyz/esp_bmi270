...
int8_t BMI270::reset()
{
    return bmi2_soft_reset(&this->sensor);
}
...
{
    return bmi2_get_status(status, &this->sensor);
}
...
{
    return bmi2_set_remap_axes(&axes, &this->sensor);
}
...
int8_t BMI270::getSensorData()
{
    ...
    err = bmi2_get_sensor_data(&rawData, &this->sensor);
    if(err != BMI2_OK) return err;
...
int8_t BMI270::getTemperature(float* tempC)
{
    ...
    uint16_t tempRaw;
    err = bmi2_get_temperature_data(&tempRaw, &this->sensor);
    if(err != BMI2_OK) return err;
...
{
    return bmi2_set_adv_power_save(BMI2_ENABLE, &this->sensor);
}
...
int8_t BMI270::setConfigs(bmi2_sens_config* configs, uint8_t numConfigs)
{
    ...
    return bmi270_set_sensor_config(configs, numConfigs, &this->sensor);
}
...
{
    return bmi270_get_feature_data(featureData, numFeatures, &this->sensor);
}
 ...
 *      BMI2_ANY_MOconvertRawToGsScalarh pin to output on, see bmi2_hw_int_pin for possible values
 *  @return Error code (0 is success, negative is failure, positive is warning)
 */
int8_t BMI270::mapInterruptToPin(uint8_t interruptSource, bmi2_hw_int_pin pin)
{
    ...
    switch(interruptSource)
    {
        ...
        case BMI2_DRDY_INT:
        case BMI2_ERR_INT:
            return bmi2_map_data_int(interruptSource, pin, &this->sensor);
            break;
        ...
        case BMI2_WRIST_WEAR_WAKE_UP_INT:
            ...
            return bmi2_map_feat_int(interruptSource, pin, &this->sensor);
            break;
...
{
    return bmi2_set_int_pin_config(&config, &this->sensor);
}
...
{
    return bmi2_get_int_pin_config(config, &this->sensor);
}
...
{
    return bmi2_get_int_status(status, &this->sensor);
}
...
int8_t BMI270::setFIFOFlags(uint16_t flags, bool enable)
{
    ...
    err = bmi2_set_fifo_config(flags, enable, &this->sensor);
    if(err != BMI2_OK) return err;
...
{
    return bmi2_set_fifo_down_sample(sensorSelect, downSample, &this->sensor);
}
...
{
    return bmi2_set_fifo_filter_data(sensorSelect, filter, &this->sensor);
}
...
{
    return bmi2_set_fifo_self_wake_up(selfWakeUp, &this->sensor);
}
...
{
    return bmi2_set_fifo_wm(numData * bytesPerFIFOData, &this->sensor);
}
...
{
    return bmi2_get_fifo_length(length, &this->sensor);
}
...
int8_t BMI270::extractFIFOData(BMI270_SensorData* data, bmi2_fifo_frame* fifoData, uint16_t* numFrames, uint8_t sensorSelect)
{
    ...
    {
        err = bmi2_extract_accel(rawData, numFrames, fifoData, &this->sensor);
    }
    else
    {
        err = bmi2_extract_gyro(rawData, numFrames, fifoData, &this->sensor);
    }
    if(err < BMI2_OK)
    {
        free(rawData);
...
int8_t BMI270::getFIFOData(BMI270_SensorData* data, uint16_t* numData)
{
    ...
    err = bmi2_read_fifo_data(&fifoData, &this->sensor);
    if(err != BMI2_OK)
    {
        free(fifoBuffer);
        return err;
...
{
    return bmi2_set_command_register(BMI2_FIFO_FLUSH_CMD, &this->sensor);
}
...
int8_t BMI270::performAccelOffsetCalibration(uint8_t gravityDirection)
{
    ...
    return bmi2_perform_accel_foc(&gDir, &this->sensor);
}
...
{
    return bmi2_perform_gyro_foc(&this->sensor);
}
...
{
    return bmi2_nvm_prog(&this->sensor);
}
...
int8_t BMI270::selfTest()
{
    ...
    err = bmi2_perform_accel_self_test(&this->sensor);
    if(err != BMI2_OK) return err;
    ...
    err = bmi2_do_gyro_st(&this->sensor);
    if(err != BMI2_OK) return err;
...
{
    return bmi2_read_aux_man_mode(addr, data.auxData, numBytes, &this->sensor);
}
...
{
    return bmi2_write_aux_man_mode(addr, data, numBytes, &this->sensor);
}
